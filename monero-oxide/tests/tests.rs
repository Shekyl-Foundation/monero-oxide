...
fn make_dummy_bp_plus() -> Bulletproof {
  ...
  for _ in 0 .. 2 {
    monero_oxide::io::write_varint(&lr_len, &mut bp).unwrap();
    for _ in 0 .. lr_len {
      push_point(&mut bp);
    }
  }
...
fn v1_transaction_rejected() {
  ...
  monero_oxide::io::write_varint(&1u64, &mut data).unwrap();
  monero_oxide::io::write_varint(&0u64, &mut data).unwrap(); // timelock
  monero_oxide::io::write_varint(&1u64, &mut data).unwrap(); // 1 input
  data.push(255); // Gen
  monero_oxide::io::write_varint(&0u64, &mut data).unwrap(); // height 0
  monero_oxide::io::write_varint(&1u64, &mut data).unwrap(); // 1 output
  monero_oxide::io::write_varint(&100u64, &mut data).unwrap(); // amount
  data.push(2); // output type
  data.extend([0u8; 32]); // key
  monero_oxide::io::write_varint(&0u64, &mut data).unwrap(); // extra len
