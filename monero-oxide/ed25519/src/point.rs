...
impl Point {
  fn elligator2_with_uniform_bytes_input(bytes: [u8; 32]) -> Self {
    ...
    */
    let r = Two25519Residue::new(&U256::from_le_bytes(bytes));
  ...
  pub fn biased_hash(bytes: [u8; 32]) -> Self {
    Self::elligator2_with_uniform_bytes_input(Keccak256::digest(bytes).into())
  }

  /// Sample a non-biased point via a hash function.
  ///
